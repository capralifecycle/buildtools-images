        ...
        if (env.BRANCH_NAME == 'master' && !isSameImage) {
          stage("Push image to public ECR") {
            ...
            if (dependencySnapshotting) {
              def snapshot = generateDependencyGraphSnapshot("$publicImageRepo:$imageTag", "${tool.name}-${imageTag}")
...
./syft \"$artifact\" -o \"github=$snapshotFile\"
  ...
  def jobCorrelator = "${env.JOB_NAME}-${artifactId}"
  def metadata = [
    ref: ref,
    sha: sha,
    job: [
...
def uploadDependencyGraphSnapshot(snapshot) {
  withCredentials([
    ...
    def connection = new URL("https://api.github.com/repos/${repositoryOwner}/${repositoryName}/dependency-graph/snapshots").openConnection() as HttpURLConnection
    ...
    if (connection.responseCode != 200) {
      println "Request to GitHub failed with status code ${connection.responseCode}: ${connection.inputStream.text}"
      throw new Exception("Failed to update GitHub dependency graph")
    }
  }
}
